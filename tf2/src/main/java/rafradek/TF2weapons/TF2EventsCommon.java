...
public class TF2EventsCommon {
				...
				if (dayTime >= 14000 && dayTime <= 21000 && dayTime % 1000 == 0 && event.world.getCurrentMoonPhaseFactor() == 1
					...
					for (EntityPlayer player : event.world.playerEntities)
						if (player.getCapability(TF2weapons.PLAYER_CAP, null).nextBossTicks <= worldTime
								&& event.world.getEntitiesWithinAABB(EntityTF2Boss.class, player.getEntityBoundingBox().expand(200, 200, 200)).isEmpty()) {
	...
	public void stopHurt(LivingAttackEvent event) {
		if (event.getSource().getEntity() != null && event.getSource().getEntity() instanceof EntityLivingBase &&
			...
			if (!TF2weapons.canInteract(damageSource)) {
	...
	public void uber(LivingHurtEvent event) {
		...
		if (target.hasCapability(TF2weapons.WEAPONS_CAP, null)) {
			...
			if (WeaponsCapability.get(target).isFeign()) {
				...
				if(!watch.isEmpty()) {
					...
					if(TF2weapons.deadRingerTrigger)
						target.onDeath(event.getSource());
					TF2weapons.sendTracking(new TF2Message.ActionMessage(24, target), target);
					target.extinguish();
					for(EntityLiving living : target.world.getEntitiesWithinAABB(EntityLiving.class, target.getEntityBoundingBox().expandXyz(80), living -> {
						return living.getAttackTarget() == living;
					})) {
						living.setAttackTarget(null);
					}
					/*EntityPlayerMP feign=FakePlayerFactory.get((WorldServer) event.getEntityLiving().getEntityWorld(), ((EntityPlayer)target).getGameProfile());
		...
		if (target instanceof EntityTF2Character && event.getSource().getEntity() != null && event.getSource().getEntity() == event.getEntity()) {
			event.setAmount(event.getAmount() * 0.35f);
		}

		if ((target.getActivePotionEffect(TF2weapons.bonk) != null || target.getActivePotionEffect(TF2weapons.uber)!=null)
				&& !event.getSource().canHarmInCreative()) {
			event.setCanceled(true);
			if (target instanceof EntityPlayer && target.getActivePotionEffect(TF2weapons.bonk) != null) {
				target.getCapability(TF2weapons.PLAYER_CAP, null).dodgedDmg+=event.getAmount();
				if(target.getCapability(TF2weapons.PLAYER_CAP, null).dodgedDmg>100){
					((EntityPlayer)target).addStat(TF2Achievements.DODGE_DAMAGE);
				}
			}
			return;
		}
		...
		if (attacker != null && attacker.hasCapability(TF2weapons.WEAPONS_CAP, null)) {
			...
			ItemStack stack=attacker==event.getSource().getSourceOfDamage()?attacker.getHeldItemMainhand():ItemStack.EMPTY;
			crit = TF2weapons.calculateCritPre(stack, attacker);
		...
		if (!(event.getSource() instanceof TF2DamageSource)) {
			crit=TF2weapons.calculateCritPost(target,attacker,crit, ItemStack.EMPTY);
			if (crit == 1) {
				event.setAmount(event.getAmount() * 1.35f);
				TF2weapons.playSound(target, TF2Sounds.MISC_MINI_CRIT, 1.5F, 1.2F / (target.getRNG().nextFloat() * 0.2F + 0.9F));
			} else if (crit == 2) {
				event.setAmount(event.getAmount() * 2f);
				TF2weapons.playSound(target, TF2Sounds.MISC_CRIT, 1.5F, 1.2F / (target.getRNG().nextFloat() * 0.2F + 0.9F));
			}
		}
	...
	public void stopUsing(PlayerInteractEvent.RightClickBlock event) {
		if (!TF2weapons.canInteract(event.getEntityLiving())) {
			event.setCanceled(true);
		}
	...
	public void stopUsing(PlayerInteractEvent.RightClickItem event) {
		...
		if (!TF2weapons.canInteract(event.getEntityLiving()) && !(item.getItem() instanceof ItemDisguiseKit)
	...
	public void livingUpdate(final LivingEvent.LivingUpdateEvent event) {
		...
		if (living.isEntityAlive() && (living.hasCapability(TF2weapons.WEAPONS_CAP, null))) {
			...
			if (!living.world.isRemote && charging != null) {
				...
				// "+living.motionZ);
				RayTraceResult result = TF2weapons.pierce(living.world, living, start.xCoord, start.yCoord, start.zCoord, end.xCoord, end.yCoord, end.zCoord, false, 0.5f, false)
						.get(0);
			...
			if (!living.world.isRemote && living.fallDistance > 0 && living.getItemStackFromSlot(EntityEquipmentSlot.FEET) != null
				...
				for (EntityLivingBase target : living.world.getEntitiesWithinAABB(EntityLivingBase.class,
						living.getEntityBoundingBox().expand(0.25, -living.motionY, 0.25), new Predicate<EntityLivingBase>() {
			...
			if (cap.isInvisible()) {
				...
				if (!visible) {
					List<Entity> closeEntities = living.world.getEntitiesInAABBexcluding(living, living.getEntityBoundingBox().expand(1, 2, 1),
		...
		if (living.getAITarget() != null && living.getAITarget().hasCapability(TF2weapons.WEAPONS_CAP, null)
				&& living.getAITarget().getCapability(TF2weapons.WEAPONS_CAP, null).invisTicks >= 20) {
	...
	public void loadWorld(WorldEvent.Load event) {
		...
		if (!event.getWorld().isRemote && event.getWorld().getScoreboard().getTeam("RED") == null) {
			...
			teamRed.setNamePrefix(TextFormatting.RED.toString());
			teamBlu.setNamePrefix(TextFormatting.BLUE.toString());
		...
		if (!event.getWorld().isRemote && event.getWorld().getScoreboard().getTeam("TF2Bosses") == null) {
			...
			teamBosses.setNamePrefix(TextFormatting.DARK_PURPLE.toString());
	...
	public void livingDeath(LivingDeathEvent event) {
		...
		if (!event.getEntity().world.isRemote && event.getSource() != null && event.getSource().getEntity() != null
				&& event.getSource().getEntity() instanceof EntityLivingBase) {
			ItemStack stack = ItemStack.EMPTY;
			final EntityLivingBase living = (EntityLivingBase) event.getSource().getEntity();
			if (event.getSource() instanceof TF2DamageSource) {
			...
			if(living instanceof EntityPlayerMP && TF2weapons.isEnemy(living, event.getEntityLiving())){
				...
				if(living.getCapability(TF2weapons.WEAPONS_CAP, null).isExpJump()){
				...
				if(event.getEntityLiving() instanceof EntitySpy && WeaponsCapability.get(event.getEntityLiving()).isInvisible()
				...
				if(!stack.isEmpty() && ItemFromData.getData(stack).getName().contains("sentry")){
					...
					if(event.getEntity() instanceof EntityTF2Character)
						player.getCapability(TF2weapons.PLAYER_CAP, null).completeObjective(Objective.KILL_W_SENTRY,stack);
				...
				if(event.getEntity() instanceof EntityTF2Character) {
					if((player.getCapability(TF2weapons.PLAYER_CAP, null).fastKillTimer+=120)>360) {
						player.getCapability(TF2weapons.PLAYER_CAP, null).fastKillTimer=0;
						player.getCapability(TF2weapons.PLAYER_CAP, null).completeObjective(Objective.KILLS,stack);
					}
					player.getCapability(TF2weapons.PLAYER_CAP, null).completeObjective(Objective.KILL,stack);
					if(event.getSource() instanceof TF2DamageSource && (((TF2DamageSource)event.getSource()).getAttackFlags()&TF2DamageSource.HEADSHOT)==TF2DamageSource.HEADSHOT) {
						player.getCapability(TF2weapons.PLAYER_CAP, null).completeObjective(Objective.HEADSHOT,stack);
					}
					if(event.getSource() instanceof TF2DamageSource && (((TF2DamageSource)event.getSource()).getAttackFlags()&TF2DamageSource.BACKSTAB)==TF2DamageSource.BACKSTAB) {
						player.getCapability(TF2weapons.PLAYER_CAP, null).completeObjective(Objective.BACKSTAB,stack);
					}
					if(event.getSource().getSourceOfDamage() instanceof EntityProjectileBase && ((EntityProjectileBase)event.getSource().getSourceOfDamage()).reflected) {
						player.getCapability(TF2weapons.PLAYER_CAP, null).completeObjective(Objective.KILL_REFLECTED,stack);
					}
					if(stack.getItem() instanceof ItemStickyLauncher) {
						player.getCapability(TF2weapons.PLAYER_CAP, null).completeObjective(Objective.STICKY_KILL,stack);
					}
					if(stack.getItem() instanceof ItemMeleeWeapon) {
						player.getCapability(TF2weapons.PLAYER_CAP, null).completeObjective(Objective.KILL_MELEE,stack);
					}
					if(player.getCapability(TF2weapons.WEAPONS_CAP, null).isExpJump()) {
						player.getCapability(TF2weapons.PLAYER_CAP, null).completeObjective(Objective.KILL_BLAST,stack);
					}
					if(player.getCapability(TF2weapons.WEAPONS_CAP, null).doubleJumped) {
						player.getCapability(TF2weapons.PLAYER_CAP, null).completeObjective(Objective.KILL_DOUBLE,stack);
					}
				}
				if(event.getEntity() instanceof EntityScout)
					player.getCapability(TF2weapons.PLAYER_CAP, null).completeObjective(Objective.KILL_SCOUT,stack);
				else if(event.getEntity() instanceof EntityPyro)
					player.getCapability(TF2weapons.PLAYER_CAP, null).completeObjective(Objective.KILL_PYRO,stack);
				else if(event.getEntity() instanceof EntitySoldier)
					player.getCapability(TF2weapons.PLAYER_CAP, null).completeObjective(Objective.KILL_SOLDIER,stack);
				else if(event.getEntity() instanceof EntityHeavy)
					player.getCapability(TF2weapons.PLAYER_CAP, null).completeObjective(Objective.KILL_HEAVY,stack);
				else if(event.getEntity() instanceof EntityDemoman)
					player.getCapability(TF2weapons.PLAYER_CAP, null).completeObjective(Objective.KILL_DEMOMAN,stack);
				else if(event.getEntity() instanceof EntityEngineer)
					player.getCapability(TF2weapons.PLAYER_CAP, null).completeObjective(Objective.KILL_ENGINEER,stack);
				else if(event.getEntity() instanceof EntityMedic)
					player.getCapability(TF2weapons.PLAYER_CAP, null).completeObjective(Objective.KILL_MEDIC,stack);
				else if(event.getEntity() instanceof EntitySpy)
					player.getCapability(TF2weapons.PLAYER_CAP, null).completeObjective(Objective.KILL_SPY,stack);
				else if(event.getEntity() instanceof EntitySniper)
					player.getCapability(TF2weapons.PLAYER_CAP, null).completeObjective(Objective.KILL_SNIPER,stack);
			...
			if(event.getEntity() instanceof EntitySentry) {
				for(EntityPlayerMP player:living.world.getEntitiesWithinAABB(EntityPlayerMP.class, living.getEntityBoundingBox().expand(8, 8, 8), new Predicate<EntityPlayerMP>() {

					@Override
					public boolean apply(EntityPlayerMP input) {
						// TODO Auto-generated method stub
						return input.getCapability(TF2weapons.WEAPONS_CAP, null).healTarget==living.getEntityId() && input.getHeldItemMainhand().getTagCompound().getBoolean("Activated");
					}
	...
	public void pickAmmo(EntityItemPickupEvent event) {
		...
		if (stack.getItem() instanceof ItemAmmo && event.getEntityLiving().hasCapability(TF2weapons.INVENTORY_CAP, null) ) {
			...
			if (!event.getEntityLiving().getCapability(TF2weapons.INVENTORY_CAP, null).getStackInSlot(3).isEmpty()) {
				...
				if(stack.isEmpty()){
					event.getItem().setEntityItemStack(orig);
				}
