...
public class EntityGrenade extends EntityProjectileBase {
	...
	public int fuse = 46;
	private static final DataParameter<Boolean> BOMB = EntityDataManager.createKey(EntityGrenade.class,
	...
	public EntityGrenade(World p_i1756_1_, EntityLivingBase p_i1756_2_, EnumHand hand) {
		...
		this.setSize(0.3f, 0.3f);
		if(TF2Attribute.getModifier("Weapon Mode", this.usedWeapon, 0, p_i1756_2_)==1){
			this.setBomb();
	...
	public void setBomb(){
		this.dataManager.set(BOMB, true);
	}
	...
	public void onUpdate() {
		...
		this.fuse--;
		if (this.fuse <= 0)
			this.explode(this.posX, this.posY + this.height / 2, this.posZ, null, this.isBomb()?1:0.64f);
		if (this.isCollided) {
			this.hitGround = true;
			if (!this.world.isRemote) {
				int attr = (int) TF2Attribute.getModifier("Coll Remove", this.usedWeapon, 0, this.shootingEntity);
				if (attr == 2)
					this.explode(this.posX, this.posY, this.posZ, null, this.isBomb()?1:0.64f);
				if (attr == 1)
					this.setDead();
			}
		}
