	...
	public void onLivingUpdate() {
		...
		if(!world.isRemote && this.begin<=0){
			...
			if(this.getAttackTarget()==null && this.ticksExisted%5==0){
				...
				if(this.scareTick<=0){
					boolean one=false;
					List<EntityLivingBase> lists=this.world.getEntitiesWithinAABB(EntityLivingBase.class, this.getEntityBoundingBox().expand(10, 10, 10), new Predicate<EntityLivingBase>(){

						@Override
						public boolean apply(EntityLivingBase input) {
							// TODO Auto-generated method stub
							return getDistanceSqToEntity(input)<100 && !TF2weapons.isOnSameTeam(EntityHHH.this, input) && !(input.getItemStackFromSlot(EntityEquipmentSlot.HEAD).isEmpty()
			...
			if(--this.attackTick<=0){
				List<EntityLivingBase> ents=this.world.getEntitiesWithinAABB(EntityLivingBase.class, this.getEntityBoundingBox().expand(3, 1, 3), new Predicate<EntityLivingBase>(){
					...
					public boolean apply(EntityLivingBase input) {
						// TODO Auto-generated method stub
						return getDistanceSqToEntity(input)<4&&!TF2weapons.isOnSameTeam(EntityHHH.this, input) && TF2weapons.lookingAt(EntityHHH.this, 30, input.posX, input.posY+input.getEyeHeight(), input.posZ);
					}
