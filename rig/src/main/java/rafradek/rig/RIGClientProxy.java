...
public class RIGClientProxy extends RIGCommonProxy {
	...
	public void registerRender() {
		ModelLoader.setCustomModelResourceLocation(RIG.rig, 0, new ModelResourceLocation(RIG.rig.getRegistryName(), "inventory"));
		...
		for (String string : modelPaths) {
			...
			ModelBakery.registerItemVariants(RIG.cannon, model);
		}

		ModelLoader.setCustomMeshDefinition(RIG.cannon, new ItemMeshDefinition() {

			@Override
			public ModelResourceLocation getModelLocation(ItemStack p_178113_1_) {

				int type = RIG.cannon.getType(p_178113_1_);
				if (type == 0)
		...
		ModelLoader.setCustomModelResourceLocation(RIG.launchpart, 0,
				new ModelResourceLocation("rafradek_blocklauncher:launchpart", "inventory"));
		ModelLoader.setCustomModelResourceLocation(RIG.launchpartBetter, 0,
