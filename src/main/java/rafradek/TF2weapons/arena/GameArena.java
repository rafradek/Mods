...
public class GameArena implements INBTSerializable<NBTTagCompound>{
	...
	public static final DataParameter<String> PLAYER_NAME = new DataParameter<String>(0, DataSerializers.STRING);
	public static final DataParameter<Optional<UUID>> PLAYER_UUID = new DataParameter<Optional<UUID>>(1, DataSerializers.OPTIONAL_UNIQUE_ID);
	public static final DataParameter<Integer> TEAM = new DataParameter<Integer>(2, DataSerializers.VARINT);
	public static final DataParameter<Integer> POINTS = new DataParameter<Integer>(3, DataSerializers.VARINT);
	...
	public void tick() {
		if (first) {
			...
			this.first = false;
			if (!(confPos != null && this.world.getTileEntity(confPos) instanceof TileEntityGameConfigure && ((TileEntityGameConfigure) this.world.getTileEntity(confPos)).getName().equals(this.name))) {
				this.markDelete = true;
			}
		}
		if (active) {
			this.joinTeamDataCache = null;
			boolean checkAll = this.bounds == null || (this.bounds.maxX == this.bounds.minX && this.bounds.maxY == this.bounds.minY && this.bounds.maxZ == this.bounds.minZ);
			List<EntityPlayer> players = checkAll ? world.playerEntities : world.getEntitiesWithinAABB(EntityPlayer.class, bounds);
			for (EntityPlayer player : players) {
				if (!playerUUID.containsKey(player.getUniqueID()) && (TF2PlayerCapability.get(player).getGameArena() == null || TF2PlayerCapability.get(player).getGameArena() == this)) {
			...
			while(itePly.hasNext()) {
				...
				if (this.world.getPlayerEntityByUUID(entry.getKey()) != player && this.world.getPlayerEntityByUUID(entry.getKey()) != null) {
				...
				if (this.world.getPlayerEntityByUUID(entry.getKey()) == null || (!checkAll && !player.getEntityBoundingBox().intersects(this.bounds))){
					itePly.remove();
				...
				if (player.isSpectator() && player.getTeam() != null && playercap.getRespawnTime() <= 0 && (cap.getUsedToken() != -1 || (this.defaultClass == -1 && !this.showClassSelection))) {
				...
				if (this.defaultClass != -1 && !cap.forcedClass) {
					((ItemToken)TF2weapons.itemToken).updateAttributes(new ItemStack(TF2weapons.itemToken, 1, this.defaultClass), player);
	...
	public void placePlayerAtSpawn(EntityPlayer player) {
		...
		if (blockpos != null)
        {
            ...
            if (blockpos1 != null)
            {
                player.setLocationAndAngles((double)((float)blockpos1.getX() + 0.5F), (double)((float)blockpos1.getY() + 0.1F), (double)((float)blockpos1.getZ() + 0.5F), 0.0F, 0.0F);
            }
        }
	...
	public void respawnPlayer(EntityPlayer player) {
		//player.setDead();
		//EntityPlayer newplayer = this.world.getMinecraftServer().getPlayerList().recreatePlayerEntity((EntityPlayerMP)player, player.dimension, false);
	...
	public void resupplyPlayer(EntityPlayer player) {
		if (this.clearInventory) {
			...
			TF2Util.removeItemsMatching(new InvWrapper(player.getCapability(TF2weapons.INVENTORY_CAP, null)), 64, stackl -> stackl.hasTagCompound() && stackl.getTagCompound().getBoolean(NBTLiterals.STACK_ARENA_ASSIGNED));
			TF2Util.removeItemsMatching(player.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null), 64, stackl -> stackl.hasTagCompound() && stackl.getTagCompound().getBoolean(NBTLiterals.STACK_ARENA_ASSIGNED));
		}
		if (WeaponsCapability.get(player).getUsedToken() != -1) {
			for (int i = 0; i < 5; i++) {
				ItemStack weapon = ItemFromData.getRandomWeaponOfSlotMob(ItemToken.CLASS_NAMES[WeaponsCapability.get(player).getUsedToken()], i,player.getRNG(), false, 0xFFFFFFFF, false);
				if (!weapon.isEmpty()) {
	...
	public void changePlayerTeam(EntityPlayer player, Team team) {
		if (this.playerUUID.containsKey(player.getUniqueID()) && this.allowedTeams.contains(team) && player.getTeam() != team) {
	...
	public void readConfig(NBTTagCompound tag) {
		if (tag.hasKey("Min Bounds")) {
			...
			this.bounds = new AxisAlignedBB(boundsMin[0],boundsMin[1],boundsMin[2], boundsMax[0], boundsMax[1], boundsMax[2]);
		}
