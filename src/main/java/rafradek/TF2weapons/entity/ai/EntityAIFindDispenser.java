...
public class EntityAIFindDispenser extends EntityAIBase {
	...
	public boolean shouldExecute() {
		if (this.host.getAmmo() <= 0 || (this.host.getAttackTarget() == null
				&& (this.host.getHealth() < this.host.getMaxHealth() || !this.host.isAmmoFull()))) {
	...
	public boolean shouldContinueExecuting() {
		return this.target != null && target.isEntityAlive()
				&& !(this.host.isAmmoFull() && this.host.getHealth() >= this.host.getMaxHealth());
	}
