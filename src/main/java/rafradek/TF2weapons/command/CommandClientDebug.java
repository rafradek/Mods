...
public class CommandClientDebug extends CommandBase {
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		if (args.length < 1) {
			...
			if ("start".equals(args[0])) {
				...
				Minecraft.getMinecraft().gameSettings.showDebugProfilerChart = true;
				...
				Minecraft.getMinecraft().gameSettings.showDebugProfilerChart = false;
				notifyCommandListener(sender, this, "commands.debug.stop",
						new Object[] { String.format("%.2f", k / 1000.0F), l });
			}
		}
	}

	private void saveProfilerResults(long timeSpan, int tickSpan, MinecraftServer server) {
		File file1 = new File(server.getFile("debug"),
	...
	public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args,
		...
		return args.length == 1 ? getListOfStringsMatchingLastWord(args, new String[] { "start", "stop" })
