...
public class GuiDisguiseKit extends GuiScreen {
	...
	public void initGui() {
		...
		for (ResourceLocation entry : ForgeRegistries.ENTITIES.getKeys()) {
			...
			if (entity instanceof EntityLivingBase
					&& ((entity.width + entity.height < 6 && entity.isNonBoss() && entity instanceof EntityCreature)
		...
		for (int i = 0; i < 16; i++) {
			this.buttonList.add(new GuiButton(i, this.width / 2 - 135 + (i % 2) * 70, this.height / 2 - 60 + i / 2 * 20,
					70, 20,
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		if (button.enabled) {
			...
			if (button.id < 16) {
				TF2weapons.network.sendToServer(new TF2Message.DisguiseMessage(
						"M:" + EntityList.getKey(mobList.get(button.id + this.firstIndex)).toString()));
			}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		this.drawCenteredString(this.fontRenderer, I18n.format("gui.disguise.info", new Object[0]), this.width / 2 - 5,
				20, 16777215);
		/*
