...
public class RenderJar extends Render<EntityJar> {
	...
	private static final ResourceLocation texture = new ResourceLocation(TF2weapons.MOD_ID,
			"textures/entity/projectile/jarate.png");

	public RenderJar(RenderManager manager) {
		super(manager);
	...
	@Override
	protected ResourceLocation getEntityTexture(EntityJar entity) {
	...
	public void doRender(EntityJar entity, double x, double y, double z, float yaw, float partialTick) {
		...
		GL11.glRotatef(
				...
				1.0F, 0.0F, 0F);
		/*
		 * GL11.glEnable(GL11.GL_BLEND); OpenGlHelper.glBlendFunc(770, 771, 1, 0);
		 */
		bindEntityTexture(entity);

		if (entity.isGas() && !entity.isGasExploded())
			Minecraft.getMinecraft().getRenderItem().renderItem(ItemFromData.getNewStack("gaspasser"),
