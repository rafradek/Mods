...
public class BlockRobotDeploy extends BlockContainer {
	...
	public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer,
		...
		if (placer instanceof EntityPlayer) {
			...
			for (EnumFacing facing : EnumFacing.HORIZONTALS) {
				...
				if (entoff instanceof TileEntityRobotDeploy && !worldIn.getBlockState(offpos).getValue(JOINED)
						&& worldIn.getBlockState(offpos).getValue(HOLDER)) {
					...
					worldIn.setBlockState(offpos,
							worldIn.getBlockState(offpos).withProperty(FACING, placefacing).withProperty(JOINED, true));
					if (worldIn.getBlockState(offpos.up()).getBlock() == this)
						worldIn.setBlockState(offpos.up(), worldIn.getBlockState(offpos.up())
			...
			/*
			 * TileEntity entbelow = worldIn.getTileEntity(pos.down()); TileEntity entup =
			 * worldIn.getTileEntity(pos.up()); IBlockState statebelow =
			 * worldIn.getBlockState(pos.down()); IBlockState stateup =
			 * worldIn.getBlockState(pos.up()); if (entbelow instanceof
			 * TileEntityRobotDeploy && !worldIn.getBlockState(pos.down()).getValue(JOINED))
			 ...
			 * worldIn.setTileEntity(pos, null); worldIn.setBlockState(pos.down(),
			 * statebelow = statebelow.withProperty(JOINED, true)); } if (entup instanceof
			 * TileEntityRobotDeploy && !worldIn.getBlockState(pos.up()).getValue(JOINED)) {
			 * worldIn.setBlockState(pos, state.withProperty(JOINED, true)); NBTTagCompound
			 * tag=new NBTTagCompound();
			 * ((TileEntityRobotDeploy)ent).readFromNBT(entup.writeToNBT(tag));
			 * worldIn.setBlockState(pos.up(), stateup = stateup.withProperty(HOLDER,
			 * false)); worldIn.setTileEntity(pos.up(), null);
			 * worldIn.setBlockState(pos.up(), stateup = stateup.withProperty(JOINED,
			 * true)); }
			 */
	...
	public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
		...
		if (state.getValue(JOINED)) {
			if (state.getValue(HOLDER) || (worldIn.getBlockState(pos.down()).getBlock() == this
					&& worldIn.getBlockState(pos.down()).getValue(HOLDER)))
